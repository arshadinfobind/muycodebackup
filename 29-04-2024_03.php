...
<body>
    ...
    if (isset($_GET['submit'])) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            echo "<div>";
            if ($page > 1) {
                echo "<a href='?page=".($page - 1)."&start_date=$start_date&end_date=$end_date&email=$email&transaction_id=$transaction_id&payment_ref=$payment_ref&submit='>Previous</a>";
            }
            for ($i = 1; $i <= $total_pages; $i++) {
                echo "<a href='?page=".$i."&start_date=$start_date&end_date=$end_date&email=$email&transaction_id=$transaction_id&payment_ref=$payment_ref&submit='>".$i."</a>";
            }
            if ($page < $total_pages) {
                echo "<a href='?page=".($page + 1)."&start_date=$start_date&end_date=$end_date&email=$email&transaction_id=$transaction_id&payment_ref=$payment_ref&submit='>Next</a>";
            }
            echo "</div>";
        } else {
