...
<head>
    ...
    <style type="text/css" media="all">
     ...
     <!-- <script type="text/javascript">
        var Data = {
            showRows: function (elem) {
                $('table.table tr.data').hide().filter(function () {
                    var rowsPrimary = $(this).data('rows-primary');
                    var rowsSecondary = $(this).data('rows-secondary');
                    return rowsPrimary !== rowsSecondary;
                }).show();
...
<div class="compare-database-block">
    ...
    <table class="table">
        <tr class="panel">
            ...
            <td class="sp">
                ...
                <!-- <a href="#" onclick="Data.showRows(this); return false;">changed rows</a> -->
                <!-- <a href="#" onclick="Data.showRows(this); return false;">changed rows</a> -->
                <a href="#" onclick="Data.showRows(this); return false;">changedRow</a>
            </td>
        </tr>
    </table>
    <table class="table">
    <tr class="header">
        ...
        $sql_primary = "SELECT TABLE_NAME, TABLE_ROWS FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '" . DATABASE_NAME . "'";
$result_primary = $conn_primary->query($sql_primary);
while ($row = $result_primary->fetch_assoc()) {
    $additionalTableInfo_primary[$row['TABLE_NAME']] = ['rows' => $row['TABLE_ROWS']];
}

// For secondary database
$sql_secondary = "SELECT TABLE_NAME, TABLE_ROWS FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = '" . DATABASE_NAME_SECONDARY . "'";
$result_secondary = $conn_secondary->query($sql_secondary);
while ($row = $result_secondary->fetch_assoc()) {
    $additionalTableInfo_secondary[$row['TABLE_NAME']] = ['rows' => $row['TABLE_ROWS']];
}
